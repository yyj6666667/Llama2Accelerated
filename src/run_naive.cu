...
void matmul(float* xout, float* x, float* w, int n, int d) {
    float *d_x = NULL, *d_w = NULL, *d_out = NULL;
    size_t bytes_x   = n * sizeof(float);
    size_t bytes_w   = (size_t)n * d * sizeof(float);
    ...
    cudaMemcpy(d_w, w, bytes_w, cudaMemcpyHostToDevice);


    int n_threads = 256;
    matmulkernel<<<d, n_threads, n_threads * sizeof(float)>>>
    (d_out, d_x, d_w, n, d);

    cudaMemcpy(xout, d_out, bytes_out, cudaMemcpyDeviceToHost);
    cudaFree(d_x); 
    cudaFree(d_w); 
    cudaFree(d_out);
}
