...
void vediomem_free(GpuCtx* ctx) {
    cudaFree(ctx->d_token_embedding_table);
    cudaFree(ctx->d_wq);
    cudaFree(ctx->d_wk);
    cudaFree(ctx->d_wv);
    cudaFree(ctx->d_wo);
...
void malloc_run_state(RunState* s, Config* p) {
    ...
    //yyj : 没错就是这部分最头疼, vediomem_init 的实现主要参考它
    // 瓶颈在gpu cpu io
    // 那么一次性全部读入！！！
...
void build_transformer(Transformer *t, char* checkpoint_path) {
    ...
    //yyj new add 
    vediomem_init(&t->gpuctx, &t->weights, &t->config, NMAX, DMAX);
}

void free_transformer(Transformer* t) {
    //yyj add
    vediomem_free(&(t->gpuctx));
...
// 现在传进来的就是cuda指针!!!!!!, 可以直接传进kernel
// 只是d_x 的值还需要拷贝， d_w 在vediomem_init 处就已经把值写好了
// 降低io 开销到原来的 2 / (n + 2) !
// version2 只有 d_x 和 d_out 有 io 开销
void matmul(GpuCtx* ctx, float* xout, float* x, float* d_w, int n, int d,
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for(unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        GpuCtx* gpuctx_ptr = &(transformer->gpuctx);
        // qkv matmuls for this position
        matmul(gpuctx_ptr, s->q, s->xb, gpuctx_ptr->d_wq + l * dim * dim, dim, dim);      //1 of 8
        matmul(gpuctx_ptr, s->k, s->xb, gpuctx_ptr->d_wk + l * dim * kv_dim, dim, kv_dim);
        matmul(gpuctx_ptr, s->v, s->xb, gpuctx_ptr->d_wv + l * dim * kv_dim, dim, kv_dim);
                    // 这里的偏移开始忘了
        ...
        matmul(&transformer->gpuctx, s->xb2, s->xb, transformer->gpuctx.d_wo + l * dim * dim, dim, dim);   // 4 of 8
        ...
        matmul(&transformer->gpuctx, s->hb, s->xb, transformer->gpuctx.d_w1 + l * dim * hidden_dim, dim, hidden_dim);
        matmul(&transformer->gpuctx, s->hb2, s->xb,transformer->gpuctx.d_w3 + l * dim * hidden_dim, dim, hidden_dim);
        ...
        matmul(&transformer->gpuctx, s->xb, s->hb, transformer->gpuctx.d_w2 + l * dim * hidden_dim, hidden_dim, dim); // 7 of 8
    ...
    matmul(&transformer->gpuctx, s->logits, x, transformer->gpuctx.d_wcls, p->dim, p->vocab_size);
    return s->logits;
}
